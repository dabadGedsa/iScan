...
Public Class clsTWAIN
    'Public Shared Function DibToImage(ByVal hdib As System.IntPtr) As Image
    '    Dim data(DIB_Size(hdib) + 100) As Byte
    '    EZTwain.DIB_WriteToBuffer(hdib, clsTWAIN.TWFF_BMP, data(0), data.Length)
    ...
    Function scan_empezar(ByVal manejador As IntPtr) As Image
        'Me.Handle.ToInt32

        Dim imagen As Image = Nothing
        Dim nPixTypes As Object = Nothing
        'Const CF_BITMAP As Short = 2
        Dim vbCFDIB As Short = 2
        Try
            My.Computer.Clipboard.Clear()
            Try
                Dim numero As Integer = AcquireToClipboard(manejador, 0&)
                'MsgBox("Ha salido el numero: " & numero)
                If numero = 0 Then
                    imagen = Nothing
                    'MsgBox("No se ha escaneado ninguna imagen", MsgBoxStyle.Critical, "Incidencia de aplicacion")
                Else
                    imagen = My.Computer.Clipboard.GetImage
                End If
            ...
            'If Err.Number = 53 Then MsgBox("No se encuentra a EZTW32.DLL, y no se puede acceder al servicio TWAIN", 48, "No se encuentra la Dll")
        Catch ex As Exception
            MsgBox(ex.Message, MsgBoxStyle.Critical, "Incidencia de aplicacion")
        End Try
    ...
    Sub scan_configurar(ByVal manejador As IntPtr)
        'Me.Handle.ToInt32
        Try
            SelectImageSource(manejador)
        Catch ex As Exception
            MsgBox("No se encuentra a EZTW32.DLL, y no se puede acceder al servicio TWAIN" & vbCrLf & ex.Message, 48, "No se encuentra la Dll")
        End Try
